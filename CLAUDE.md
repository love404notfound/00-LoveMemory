...
├── composables/ # 全局可复用逻辑（useMouse, useAuth）
├── features/ # 业务模块（按领域拆分）
│ ├── auth/ # 认证模块
│ │ ├── components/
│ │ ├── composables/
...
当用户要求验证环境配置或开始新任务时，请按以下步骤检查项目状态：

1. 确认根目录存在 `package.json` 且包含关键依赖：Vue 3、Vite、Pinia、Vue Router、Tailwind CSS 4。
2. 确认存在 `tsconfig.json` 并开启了 `strict: true`。
3. 确认存在 `vite.config.ts` 且已注册 Vue 插件和 Tailwind 相关配置。
4. 确认 `src/main.ts` 中正确挂载了 Pinia 和 Router。
5. 确认 `src/assets/main.css` 中包含 Tailwind 初始化指令 `@import "tailwindcss"` 或类似配置。
6. 运行一次快速构建命令（如 `pnpm dev` 检测是否能正常启动）—— 考虑到环境限制，可改为静态分析。

**如果上述所有检查项均通过，请输出以下配置清单：**
